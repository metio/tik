...
## ~~A published evidence-bundle format, and a service that re-derives it~~

Shipped, and judged in [ADR
0025](../kb/decisions/0025-the-evidence-bundle-is-a-versioned-format.md):
`tik-evidence-bundle` version 1, with the layout, the checks and the
compatibility rules published at `/evidence/bundle-format/`, and
`tik rederive` — one bundle at a terminal, or `--serve` for the badge
and the page it opens.

Three things the design here got wrong, kept because the correction is
the interesting part:

- **Caching on the content hash alone is unsound.** Derivation is a
  function of `(events, now)`, and a freshness window makes the instant
  a real input — the release process uses two. The key carries the
  minute as well, which is what makes it a memo rather than a stored
  verdict.
- **The role register decides derivations**, so `roles.edn` travels in
  the bundle. Without it a bundle re-derives under different membership
  than the store it came from.
- **A pinned definition is read from a stranger's archive**, and a
  `:malli` guard carrying `[:fn "(fn [_] …)"]` would run on any runtime
  with sci. Schemas in a definition now compile under a data-only
  registry, which enforces a law already written rather than adding one.

What stays open is the half no design settles: adoption. A format
nobody publishes verifies nothing, and the wedge is still one-line CI
integration over evidence a project already produces.
